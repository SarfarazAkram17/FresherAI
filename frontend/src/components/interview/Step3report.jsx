      ...
      >
        <div className="border-b border-white/10 px-8 py-6 flex items-center justify-between">
          <div>
            ...
            <DownloadBtn docRef={reportRef} setUser={setUser} />
          </div>
              ...
              <div className="mt-5 space-y-3">
                {report.strengths?.length > 0 ? (
                  report.strengths?.map((s) => (
                    ...
                    >
                      <div className="w-6 h-6 rounded-full bg-green-500 flex items-center justify-center shrink-0 text-xs">
                        <FiCheck size={16} />
                      </div>
              ...
              <div className="mt-5 space-y-3">
                {report.weaknesses?.length > 0 ? (
                  report.weaknesses?.map((w) => (
                    ...
                    >
                      <div className="w-6 h-6 rounded-full bg-red-500 flex items-center justify-center shrink-0 text-xs">
                        !
                      </div>
                      <p className="text-sm text-zinc-300 leading-6">{w}</p>
                    </div>
                  ))
                ) : (
                  <p className="text-zinc-500 text-sm">
            ...
            <div className="mt-5 space-y-3">
              {report.recommendations?.length > 0 ? (
                report.recommendations?.map((r, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-3.5 rounded-xl bg-white/3 border border-white/10 p-4"
                  >
                    <div className="w-7 h-7 rounded-full bg-white text-black flex items-center justify-center font-semibold shrink-0 text-sm">
                      {i + 1}
                    </div>
                ...
                >
                  ...
                  <div className="grid md:grid-cols-4 gap-3.5 mt-6">
                    <div className="rounded-xl bg-black/20 p-3.5">
                      ...
                      <h5 className="mt-2 text-left font-bold text-white">
                        {item.feedback?.score}
                      </h5>
                    </div>
                  ...
                  <div className="mt-6 rounded-xl border border-green-500/20 bg-green-500/5 p-4.5">
                    ...
                    <p className="mt-2.5 text-sm leading-6 text-zinc-300">
                      {item.feedback?.feedback || "No feedback available."}
                    </p>
                  </div>

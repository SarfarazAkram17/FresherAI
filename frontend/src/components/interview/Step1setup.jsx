...
const Step1setup = ({ user, setUser }) => {
  ...
  const start = async () => {
    setStarting(true);
    const res = await startInterview({ role, type, useResume, resume });
        ...
        <div className="p-5 sm:p-7 flex flex-col">
          ...
          <div className="mt-5 flex-1 space-y-4 overflow-y-auto">
            ...
            <div>
              ...
              <div className="mt-1.5 flex rounded-xl bg-[#17181E] border border-white/10 p-1">
                {["technical", "hr"].map((item) => (
                  <button
                    key={item}
                    onClick={() => setType(item)}
                    className={`flex-1 h-9 rounded-lg text-xs cursor-pointer sm:text-sm font-medium capitalize transition-all ${type === item ? "bg-white text-black" : "text-zinc-400 hover:text-white"}`}
                  >
                    {item}
                  </button>
                ))}
            ...
            <div className="rounded-xl border border-white/10 bg-[#17181E] p-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  className={`relative shrink-0 w-12 h-7 rounded-full cursor-pointer transition ${useResume ? "bg-white" : "bg-zinc-700"}`}
            ...
            {useResume && (
              ...
              >
                ...
                {file && (
                  <div className="mt-4">
                    <div className="rouded-lg bg-black/20 border border-white/10 p-2.5">
                    ...
                    <button
                      ...
                      disabled={uploading}
                      className="mt-3 w-full h-10 rounded-xl bg-white cursor-pointer text-black text-sm font-semibold hover:opacity-90 transition disabled:opacity-60"
                    >
          ...
          <motion.button
            ...
            className="mt-5 h-12 rounded-xl cursor-pointer bg-white text-black text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-40 transition"
          >
            {starting ? (
              "Starting Interview..."
            ) : (

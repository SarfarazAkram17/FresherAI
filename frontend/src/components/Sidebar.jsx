...
const Sidebar = ({
  ...
  const inner = (
    <div className="flex flex-col h-full">
      <div
        className={`px-3 h-13 border-b border-black/8 shrink-0 flex items-center ${sidebarOpen ? "justify-between" : "justify-center"}`}
      >
        {sidebarOpen && (
        ...
        <div className="flex items-center">
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.95 }}
            className="md:hidden cursor-pointer text-black/30 hover:text-[#0A0A0A] transition-colors shrink-0"
          >
            <FiSidebar size={15} />
          </motion.button>
        </div>
      ...
      <div className="px-2.5 pt-3 pb-1.5 shrink-0">
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          className={`w-full flex items-center gap-2 bg-[#000000] text-white font-semibold rounded-lg py-2 transition-all shadow-[0_4px_14px_rgba(0,0,0,0.25)] hover:bg-[#1a1a1a] cursor-pointer ${sidebarOpen ? "px-2.5" : "justify-center px-0"}`}
        >
      ...
      <nav className="flex flex-col gap-0.5 px-2.5 flex-1">
        {NAV_ITEMS.map((nav, i) => (
          <motion.button
            ...
            className={`flex items-center gap-2.5 rounded-lg py-2 cursor-pointer transition-all text-xs font-medium text-black/45 hover:text-[#0A0A0A] hover:bg-black/5 ${sidebarOpen ? "px-2.5" : "justify-center px-0"}`}
          >
          ...
          <AnimatePresence>
            {sidebarOpen && (
              ...
              >
                <p
                  title={user?.name}
                  className="text-[#0A0A0A] text-[11px] font-semibold truncate"
                >
                  {user?.name ?? "User"}
                </p>
                <p
                  title={user?.email}
                  className="text-black/35 text-[9px] truncate"
                >
          ...
          <AnimatePresence>
            {sidebarOpen && (
              <motion.button
                ...
                className="text-black/30 hover:text-[#0A0A0A] transition-colors ml-auto cursor-pointer"
              >
                <FiLogOut size={13} />
              </motion.button>
            )}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.aside
            ...
            exit={{ x: -280 }}
            transition={{ duration: 0.25, ease: "easeInOut" }}
            className="fixed top-0 left-0 h-screen w-70 max-w-[85vw] bg-white border-r border-black/8 flex flex-col z-50 md:hidden overflow-hidden"
          >
            {inner}
          </motion.aside>
        )}

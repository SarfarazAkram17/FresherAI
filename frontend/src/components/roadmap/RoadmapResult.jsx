...
const RoadmapResult = ({ roadmap, onClear }) => {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      >
        ...
        <div className="relative flex items-start justify-between mb-4">
          <div>
            <p className="text-xs text-white/35 tracking-widest uppercase mb-1">
              Your Roadmap
            </p>
            <h2 className="text-xs font-bold text-white">{roadmap.title}</h2>
          ...
          <button
            className="text-white/30 hover:text-white/70 transition-colors mt-0.5 cursor-pointer"
            onClick={onClear}
          >
            <FiX size={16} />
          </button>
        ...
        <div className="relative grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
          {[
            ...
            { icon: FiClock, label: "Duration", value: roadmap.duration },
            {
              icon: FiCheckCircle,
              label: "Modules",
              value: `${roadmap.modules.length} topics`,
            },

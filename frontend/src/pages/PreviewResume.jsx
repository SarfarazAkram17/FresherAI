  ...
  return (
    <div className="min-h-screen bg-white text-[#0A0A0A]">
      ...
      <div className="sticky top-0 z-20 border-b border-black/8 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
          <div>
          ...
          <div className="flex items-center justify-between lg:justify-end gap-2.5">
            <button
              onClick={onBack}
              className="flex h-8 items-center justify-center gap-1.5 rounded-lg border border-black/15 px-2.5 sm:px-3 text-xs text-black/60 trnasition hover:border-black/35 hover:text-[#0A0A0A] cursor-pointer"
            >
              <FiArrowLeft size={15} />
              <span className="hidden sm:block">Back to Edit</span>
            </button>

            <DownloadBtn docRef={resumeRef} setUser={setUser} />
          </div>
        </div>
      </div>
      ...
      <div className="overflow-auto bg-[#F8F9FA] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mx-auto flex justify-center">
          ...
          >
            <div
              ref={resumeRef}
              className="rounded-md bg-white shadow-[0_0_50px_rgba(0,0,0,0.6)]"
            >
